...
public class BaseDaoImpl<T> extends SqlSessionDaoSupport implements BaseDao<T> {
	...
	public void deleteMore(Class<?> cls, String ids) {
		//此处ids.split(",")变成数组传入xxx.xml中可以匹配array
	...
	public List<T> queryAll(Class<?> cls, T t) {
		...
		return getSqlSession().selectList(cls.getSimpleName()+".queryAll",t);
	}
	...
	public List<T> queryByPageCondition(Class<?> cls, Map<String, Object> map) {
		...
		return getSqlSession().selectList(cls.getSimpleName()+".queryByPageCondition",map);
	}
	...
	public int getTotalsCondition(Class<?> cls, String condition) {
		...
		return getSqlSession().selectOne(cls.getSimpleName()+".getTotalsCondition");
	}
